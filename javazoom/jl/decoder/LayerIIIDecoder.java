...
final class LayerIIIDecoder implements FrameDecoder {
   ...
   public LayerIIIDecoder(Bitstream stream0, Header header0, SynthesisFilter filtera, SynthesisFilter filterb, Obuffer buffer0, int which_ch0) {
      ...
      if(reorder_table == null) {
         ...
         for(int ll0 = 0; ll0 < 9; ++ll0) {
            reorder_table[ll0] = reorder(this.sfBandIndex[ll0].s);
         }
      }

      int[] var29 = new int[]{0, 6, 11, 16, 21};
      int[] ss0 = new int[]{0, 6, 12};
      this.sftable = new LayerIIIDecoder.Sftable(var29, ss0);
   ...
   private void get_scale_factors(int ch, int gr) {
      ...
      if(gr_info.window_switching_flag != 0 && gr_info.block_type == 2) {
         if(gr_info.mixed_block_flag != 0) {
            ...
            for(window = 0; window < 3; ++window) {
               this.scalefac[ch].s[window][var9] = 0;
            }
         } else {
      ...
      if(gr_info.window_switching_flag != 0 && gr_info.block_type == 2) {
         ...
         if(gr_info.mixed_block_flag != 0) {
            ...
            for(int sfb = 3; sfb < 13; ++sfb) {
               ...
               reste = (sfb_start << 2) - sfb_start;
               int freq = 0;

               for(int freq3 = 0; freq < sfb_lines; freq3 += 3) {
                  int src_line = reste + freq;
                  int des_line = reste + freq3;
                  quotien = src_line % 18;
                  quotien1 = (src_line - quotien) / 18;
                  this.out_1d[des_line] = xr_1d[quotien1][quotien];
                  src_line += sfb_lines;
                  ++des_line;
                  quotien = src_line % 18;
                  quotien1 = (src_line - quotien) / 18;
            ...
            for(index = 0; index < 576; ++index) {
               reste = reorder_table[this.sfreq][index];
               quotien = reste % 18;
               quotien1 = (reste - quotien) / 18;
               this.out_1d[index] = xr_1d[quotien1][quotien];
            }
         }
      } else {
         for(index = 0; index < 576; ++index) {
      ...
      if(this.channels == 1) {
         ...
         if(i_stereo) {
            ...
            if(gr_info.window_switching_flag != 0 && gr_info.block_type == 2) {
               ...
               if(gr_info.mixed_block_flag != 0) {
                  ...
                  for(sfbcnt = 0; sfbcnt < 3; ++sfbcnt) {
                     ...
                     for(sfb = 12; sfb >= 3; --sfb) {
                        ...
                        for(i = (i << 2) - i + (sfbcnt + 1) * lines - 1; lines > 0; --i) {
                     ...
                     while(sfb < 12) {
                        ...
                        for(i = (temp << 2) - temp + sfbcnt * sb; sb > 0; --sb) {
                           this.is_pos[i] = this.scalefac[1].s[sfbcnt][sfb];
                           if(this.is_pos[i] != 7) {
                              if(lsf) {
                  ...
                  if(j <= 3) {
                     ...
                     for(i = this.sfBandIndex[this.sfreq].l[i]; sfb < 8; ++sfb) {
                  ...
                  for(j = 0; j < 3; ++j) {
                     ...
                     for(sfb = 12; sfb >= 0; --sfb) {
                        ...
                        for(i = (temp << 2) - temp + (j + 1) * lines - 1; lines > 0; --i) {
                           if(this.ro[1][i / 18][i % 18] != 0.0F) {
                              sfbcnt = sfb;
                              sfb = -10;
                              lines = -10;
                     ...
                     for(sfb = sfbcnt + 1; sfb < 12; ++sfb) {
                        ...
                        for(i = (temp << 2) - temp + j * sb; sb > 0; --sb) {
                     ...
                     for(i = (temp2 << 2) - temp2 + j * sb; sb > 0; --sb) {
                        this.is_pos[i] = this.is_pos[sfb];
                        if(lsf) {
                           this.k[0][i] = this.k[0][sfb];
                           this.k[1][i] = this.k[1][sfb];
               ...
               for(i = this.sfBandIndex[this.sfreq].l[i]; sfb < 21; ++sfb) {
                  for(sb = this.sfBandIndex[this.sfreq].l[sfb + 1] - this.sfBandIndex[this.sfreq].l[sfb]; sb > 0; --sb) {
                     this.is_pos[i] = this.scalefac[1].l[sfb];
                     if(this.is_pos[i] != 7) {
                        if(lsf) {
   ...
   public void inv_mdct(float[] in, float[] out, int block_type) {
      ...
      if(block_type == 2) {
         ...
         for(int i = 0; i < 3; ++i) {
            ...
            in[9 + i] += in[3 + i];
            tmp2 = in[12 + i] * 0.5F;
            tmp1 = in[6 + i] * 0.8660254F;
            tmp3 = in[0 + i] + tmp2;
            tmpf_1 = in[0 + i] - in[12 + i];
            tmpf_0 = tmp3 + tmp1;
            tmpf_2 = tmp3 - tmp1;
            tmp2 = in[15 + i] * 0.5F;
            tmp1 = in[9 + i] * 0.8660254F;
            tmp3 = in[3 + i] + tmp2;
         ...
         float var50 = iip12 + in[4] * 1.8793852F + in[8] * 1.5320889F + in[16] * 0.34729636F;
         tmp1 = i00 + in[4] - in[8] - in[12] - in[12] - in[16];
         tmp2 = iip12 - in[4] * 0.34729636F - in[8] * 1.8793852F + in[16] * 1.5320889F;
         tmp3 = iip12 - in[4] * 1.5320889F + in[8] * 0.34729636F - in[16] * 1.8793852F;
