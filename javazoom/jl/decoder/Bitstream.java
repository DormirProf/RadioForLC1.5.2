...
public final class Bitstream implements BitstreamErrors {
   ...
   public Bitstream(InputStream in) {
      if(in == null) {
         ...
         this.source = new PushbackInputStream(in1, 1732);
         this.closeFrame();
      }
   }
   ...
   int read_frame_data(int bytesize) throws BitstreamException {
      ...
      int numread1 = this.readFully(this.frame_bytes, 0, bytesize);
   ...
   void parse_frame() throws BitstreamException {
      ...
      for(int k = 0; k < bytesize; k += 4) {
         ...
         this.framebuffer[b++] = var10 << 24 & -16777216 | b1 << 16 & 16711680 | b2 << 8 & '\uff00' | b3 & 255;
      }
   ...
   public int get_bits(int number_of_bits) {
      ...
      int var6;
      if(sum <= 32) {
         var6 = this.framebuffer[this.wordpointer] >>> 32 - sum & this.bitmask[number_of_bits];
         ...
         var6 = Right << 16 & -65536 | Left >>> 16 & '\uffff';
         var6 >>>= 48 - sum;
         var6 &= this.bitmask[number_of_bits];
   ...
   private int readFully(byte[] b, int offs, int len) throws BitstreamException {
      ...
      try {
         while(len > 0) {
            int ex = this.source.read(b, offs, len);
            if(ex == -1) {
               while(len-- > 0) {
                  b[offs++] = 0;
               }
