...
class LayerIIDecoder extends LayerIDecoder implements FrameDecoder {
   ...
   protected void createSubbands() {
      int i;
      if(super.mode == 3) {
         for(i = 0; i < super.num_subbands; ++i) {
            super.subbands[i] = new LayerIIDecoder.SubbandLayer2(i);
         }
      } else if(super.mode == 1) {
         for(i = 0; i < super.header.intensity_stereo_bound(); ++i) {
            super.subbands[i] = new LayerIIDecoder.SubbandLayer2Stereo(i);
         }

         while(i < super.num_subbands) {
            super.subbands[i] = new LayerIIDecoder.SubbandLayer2IntensityStereo(i);
            ++i;
         }
      } else {
         for(i = 0; i < super.num_subbands; ++i) {
   ...
   protected void readScaleFactorSelection() {
      for(int i = 0; i < super.num_subbands; ++i) {
         ((LayerIIDecoder.SubbandLayer2)super.subbands[i]).read_scalefactor_selection(super.stream, super.crc);
      }
   ...
   static class SubbandLayer2Stereo extends LayerIIDecoder.SubbandLayer2 {
      ...
      public void read_allocation(Bitstream stream, Header header, Crc16 crc) {
         ...
         super.allocation = stream.get_bits(length);
         this.channel2_allocation = stream.get_bits(length);
         if(crc != null) {
            crc.add_bits(super.allocation, length);
      ...
      public void read_scalefactor_selection(Bitstream stream, Crc16 crc) {
         if(super.allocation != 0) {
            super.scfsi = stream.get_bits(2);
            if(crc != null) {
               crc.add_bits(super.scfsi, 2);
            }
         }
      ...
      public void read_scalefactor(Bitstream stream, Header header) {
         ...
         if(this.channel2_allocation != 0) {
            ...
            case 0:
               this.channel2_scalefactor1 = LayerIDecoder.Subband.scalefactors[stream.get_bits(6)];
               this.channel2_scalefactor2 = LayerIDecoder.Subband.scalefactors[stream.get_bits(6)];
               this.channel2_scalefactor3 = LayerIDecoder.Subband.scalefactors[stream.get_bits(6)];
               break;
            case 1:
               this.channel2_scalefactor1 = this.channel2_scalefactor2 = LayerIDecoder.Subband.scalefactors[stream.get_bits(6)];
               this.channel2_scalefactor3 = LayerIDecoder.Subband.scalefactors[stream.get_bits(6)];
               break;
            case 2:
               this.channel2_scalefactor1 = this.channel2_scalefactor2 = this.channel2_scalefactor3 = LayerIDecoder.Subband.scalefactors[stream.get_bits(6)];
               break;
            case 3:
               this.channel2_scalefactor1 = LayerIDecoder.Subband.scalefactors[stream.get_bits(6)];
               this.channel2_scalefactor2 = this.channel2_scalefactor3 = LayerIDecoder.Subband.scalefactors[stream.get_bits(6)];
            }
      ...
      public boolean read_sampledata(Bitstream stream) {
         ...
         if(this.channel2_allocation != 0) {
            if(super.groupingtable[1] != null) {
               ...
               float[] source = super.groupingtable[1];
               byte tmp = 0;
      ...
      public boolean put_next_sample(int channels, SynthesisFilter filter1, SynthesisFilter filter2) {
         ...
         if(this.channel2_allocation != 0 && channels != 1) {
            float sample = this.channel2_samples[super.samplenumber - 1];
            if(super.groupingtable[1] == null) {
            ...
            if(channels == 0) {
               filter2.input_sample(sample, super.subbandnumber);
            } else {
               filter1.input_sample(sample, super.subbandnumber);
            }
         }
   ...
   static class SubbandLayer2IntensityStereo extends LayerIIDecoder.SubbandLayer2 {
      ...
      public boolean put_next_sample(int channels, SynthesisFilter filter1, SynthesisFilter filter2) {
         if(super.allocation != 0) {
            float sample = super.samples[super.samplenumber];
            if(super.groupingtable[0] == null) {
               sample = (sample + super.d[0]) * super.c[0];
            }

            if(channels == 0) {
               float sample2 = sample;
               if(super.groupnumber <= 4) {
                  sample *= super.scalefactor1;
                  sample2 *= this.channel2_scalefactor1;
               } else if(super.groupnumber <= 8) {
                  sample *= super.scalefactor2;
                  sample2 *= this.channel2_scalefactor2;
               } else {
                  sample *= super.scalefactor3;
   ...
   static class SubbandLayer2 extends LayerIDecoder.Subband {
      ...
      protected int get_allocationlength(Header header) {
         if(header.version() == 1) {
            ...
            return channel_bitrate != 1 && channel_bitrate != 2?(this.subbandnumber <= 10?4:(this.subbandnumber <= 22?3:2)):(this.subbandnumber <= 1?4:3);
         } else {
            return this.subbandnumber <= 3?4:(this.subbandnumber <= 10?3:2);
         }
      }
      ...
      public void read_scalefactor(Bitstream stream, Header header) {
         if(this.allocation != 0) {
            switch(this.scfsi) {
            case 0:
               this.scalefactor1 = LayerIDecoder.Subband.scalefactors[stream.get_bits(6)];
               this.scalefactor2 = LayerIDecoder.Subband.scalefactors[stream.get_bits(6)];
               this.scalefactor3 = LayerIDecoder.Subband.scalefactors[stream.get_bits(6)];
               break;
            case 1:
               this.scalefactor1 = this.scalefactor2 = LayerIDecoder.Subband.scalefactors[stream.get_bits(6)];
               this.scalefactor3 = LayerIDecoder.Subband.scalefactors[stream.get_bits(6)];
               break;
            case 2:
               this.scalefactor1 = this.scalefactor2 = this.scalefactor3 = LayerIDecoder.Subband.scalefactors[stream.get_bits(6)];
               break;
            case 3:
               this.scalefactor1 = LayerIDecoder.Subband.scalefactors[stream.get_bits(6)];
               this.scalefactor2 = this.scalefactor3 = LayerIDecoder.Subband.scalefactors[stream.get_bits(6)];
            }
      ...
      public boolean read_sampledata(Bitstream stream) {
         if(this.allocation != 0) {
            if(this.groupingtable[0] != null) {
               ...
               ++temp;
               int var7 = tmp + 1;
               target[var7] = source[temp];
               ++temp;
               ++var7;
               target[var7] = source[temp];
            } else {
               this.samples[0] = (float)((double)((float)stream.get_bits(this.codelength[0]) * this.factor[0]) - 1.0D);
               this.samples[1] = (float)((double)((float)stream.get_bits(this.codelength[0]) * this.factor[0]) - 1.0D);
               this.samples[2] = (float)((double)((float)stream.get_bits(this.codelength[0]) * this.factor[0]) - 1.0D);
            }
         }
