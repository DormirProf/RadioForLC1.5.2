...
class LayerIDecoder implements FrameDecoder {
   ...
   static class SubbandLayer1IntensityStereo extends LayerIDecoder.SubbandLayer1 {
      ...
      public void read_scalefactor(Bitstream stream, Header header) {
         if(super.allocation != 0) {
            super.scalefactor = LayerIDecoder.Subband.scalefactors[stream.get_bits(6)];
            this.channel2_scalefactor = LayerIDecoder.Subband.scalefactors[stream.get_bits(6)];
         }
      ...
      public boolean put_next_sample(int channels, SynthesisFilter filter1, SynthesisFilter filter2) {
         if(super.allocation != 0) {
            ...
            float sample2;
            if(channels == 0) {
               sample2 = super.sample * super.scalefactor;
               float sample21 = super.sample * this.channel2_scalefactor;
               filter1.input_sample(sample2, super.subbandnumber);
               filter2.input_sample(sample21, super.subbandnumber);
            } else if(channels == 1) {
               sample2 = super.sample * super.scalefactor;
               filter1.input_sample(sample2, super.subbandnumber);
            } else {
               sample2 = super.sample * this.channel2_scalefactor;
               filter1.input_sample(sample2, super.subbandnumber);
            }
         }
   ...
   static class SubbandLayer1 extends LayerIDecoder.Subband {
      ...
      public void read_scalefactor(Bitstream stream, Header header) {
         if(this.allocation != 0) {
            this.scalefactor = LayerIDecoder.Subband.scalefactors[stream.get_bits(6)];
         }
   ...
   static class SubbandLayer1Stereo extends LayerIDecoder.SubbandLayer1 {
      ...
      public void read_allocation(Bitstream stream, Header header, Crc16 crc) throws DecoderException {
         super.allocation = stream.get_bits(4);
         this.channel2_allocation = stream.get_bits(4);
         if(crc != null) {
            crc.add_bits(super.allocation, 4);
            crc.add_bits(this.channel2_allocation, 4);
         }

         if(super.allocation != 0) {
            super.samplelength = super.allocation + 1;
            super.factor = LayerIDecoder.SubbandLayer1.table_factor[super.allocation];
            super.offset = LayerIDecoder.SubbandLayer1.table_offset[super.allocation];
         }

         if(this.channel2_allocation != 0) {
            this.channel2_samplelength = this.channel2_allocation + 1;
            this.channel2_factor = LayerIDecoder.SubbandLayer1.table_factor[this.channel2_allocation];
            this.channel2_offset = LayerIDecoder.SubbandLayer1.table_offset[this.channel2_allocation];
         }
      ...
      public boolean put_next_sample(int channels, SynthesisFilter filter1, SynthesisFilter filter2) {
         ...
         if(this.channel2_allocation != 0 && channels != 1) {
            ...
            if(channels == 0) {
               filter2.input_sample(sample2, super.subbandnumber);
            } else {
               filter1.input_sample(sample2, super.subbandnumber);
            }
         }
